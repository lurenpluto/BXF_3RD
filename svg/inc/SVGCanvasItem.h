...
class wxSVGCanvasText: public wxSVGCanvasItem
{
  ...
  protected:
	...
	virtual void InitChildren(wxSVGTextPositioningElement& element, const wxCSSStyleDeclaration& style,
			wxSVGMatrix* matrix);
	virtual void AddChunk(const std::wstring& text, const wxCSSStyleDeclaration& style, wxSVGMatrix* matrix);
    ...
    virtual void InitText(const std::wstring& text, const wxCSSStyleDeclaration& style, wxSVGMatrix* matrix) = 0;
};
...
public:
    ...
    std::wstring m_href; /** link to the image (filename) */
