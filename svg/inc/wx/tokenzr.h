...
public:
    ...
    wxStringTokenizer(const std::wstring& str,
                      const std::wstring& delims = wxDEFAULT_DELIMITERS,
    ...
    void Reinit(const std::wstring& str);
    ...
    wchar_t GetLastDelimiter() const { return m_lastDelim; }

    // get current tokenizer state
        // returns the part of the string which remains to tokenize (*not* the
        // initial string)
    std::wstring GetString() const { return m_string.substr(m_pos); }
    ...
    std::wstring NextToken() { return GetNextToken(); }

    // compatibility only, don't use
    void SetString(const std::wstring& to_tokenize,
    ...
    wxStringTokenizer(const std::wstring& to_tokenize,
                      const std::wstring& delims,
                      bool ret_delim)
    {
...
protected:
    ...
    std::wstring m_string,              // the string we tokenize
    ...
    wchar_t   m_lastDelim;           // delimiter after last token or '\0'
};
