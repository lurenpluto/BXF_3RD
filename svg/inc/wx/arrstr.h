...
#include "wx/defs.h"
#include "../src/String_wxsvg.h"

 int wxCMPFUNC_CONV wxStringSortAscending(std::wstring*, std::wstring*);
 int wxCMPFUNC_CONV wxStringSortDescending(std::wstring*, std::wstring*);

#if wxUSE_STL

#include "wx/dynarray.h"

typedef int (wxCMPFUNC_CONV *CMPFUNCwxString)(std::wstring*, std::wstring*);
typedef std::wstring _wxArraywxBaseArrayStringBase;
_WX_DECLARE_BASEARRAY_2(_wxArraywxBaseArrayStringBase, wxBaseArrayStringBase,
                        wxArray_SortFunction<std::wstring>,
                        class );
WX_DEFINE_USER_EXPORTED_TYPEARRAY(std::wstring, wxArrayStringBase,
                                  wxBaseArrayStringBase, );
_WX_DEFINE_SORTED_TYPEARRAY_2(std::wstring, wxSortedArrayStringBase,
                              wxBaseArrayStringBase, = wxStringSortAscending,
                              class , CMPFUNCwxString);

class  wxArrayString : public wxArrayStringBase
{
public:
    // type of function used by wxArrayString::Sort()
    typedef int (wxCMPFUNC_CONV *CompareFunction)(const std::wstring& first,
                                                  const std::wstring& second);

    wxArrayString() { }
    wxArrayString(const wxArrayString& a) : wxArrayStringBase(a) { }
    wxArrayString(size_t sz, const wchar_t** a);
    wxArrayString(size_t sz, const std::wstring* a);

    int Index(const wchar_t* sz, bool bCase = true, bool bFromEnd = false) const;

    void Sort(bool reverseOrder = false);
    void Sort(CompareFunction function);
    void Sort(CMPFUNCwxString function) { wxArrayStringBase::Sort(function); }

    size_t Add(const std::wstring& string, size_t copies = 1)
    {
...
// ----------------------------------------------------------------------------
// The string array uses it's knowledge of internal structure of the std::wstring
// class to optimize string storage. Normally, we would store pointers to
// string, but as std::wstring is, in fact, itself a pointer (sizeof(std::wstring) is
// sizeof(char *)) we store these pointers instead. The cast to "std::wstring *" is
...
public:
  ...
  // wxArray
  typedef int (wxCMPFUNC_CONV *CompareFunction2)(std::wstring* first,
                                  std::wstring* second);
  ...
  wxArrayString() { Init(false); }
    ...
    //     we'd like to be implicitly converted to std::wstring instead!)
    //
 ...
 ~wxArrayString();
  ...
  std::wstring& Item(size_t nIndex) const
    {
        wxASSERT_MSG( nIndex < m_nCount,
                      _T("wxArrayString: index out of bounds") );

        return *(std::wstring *)&(m_pItems[nIndex]);
    }

    // same as Item()
  std::wstring& operator[](size_t nIndex) const { return Item(nIndex); }
    // get last item
  std::wstring& Last() const
  {
    ...
    // return a std::wstring[], useful for the controls which
    // take one in their ctor.  You must delete[] it yourself
    // once you are done with it.  Will return NULL if the
    // ArrayString was empty.
#if WXWIN_COMPATIBILITY_2_4
  wxDEPRECATED( std::wstring* GetStringArray() const );
#endif
  ...
  int  Index (const wchar_t *sz, bool bCase = true, bool bFromEnd = false) const;
    ...
    // index
  size_t Add(const std::wstring& str, size_t nInsert = 1);
    // add new element at given position
  void Insert(const std::wstring& str, size_t uiIndex, size_t nInsert = 1);
    // expand the array to have count elements
  void SetCount(size_t count);
    // remove first item matching this value
  void Remove(const wchar_t *sz);
  ...
  iterator begin() { return (std::wstring *)&(m_pItems[0]); }
  const_iterator begin() const { return (std::wstring *)&(m_pItems[0]); }
...
private:
  ...
  wchar_t  **m_pItems; // pointer to data
...
// this class provides a temporary std::wstring* from a
// wxArrayString
...
public:
    ...
    std::wstring* GetStrings()
    {
        ...
        m_strings = new std::wstring[count];
