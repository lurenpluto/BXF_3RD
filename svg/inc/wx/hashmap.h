...
// std::wstring, char*, wchar_t*
class  wxStringHash
{
public:
    wxStringHash() {}
    unsigned long operator()( const std::wstring& x ) const
        { return wxCharStringHash( x.c_str() ); }
    unsigned long operator()( const wchar_t* x ) const
        { return wxCharStringHash( x ); }
    static unsigned long wxCharStringHash( const wchar_t* );
#if wxUSE_UNICODE
...
#define WX_DECLARE_STRING_HASH_MAP( VALUE_T, CLASSNAME ) \
    _WX_DECLARE_HASH_MAP( std::wstring, VALUE_T, wxStringHash, wxStringEqual, \
                          CLASSNAME, class )
