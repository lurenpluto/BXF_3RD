...
{
public:
    wxClassInfo( const wchar_t *className,
    ...
    const wchar_t       *GetBaseClassName1() const
        { return m_baseInfo1 ? m_baseInfo1->GetClassName() : NULL; }
    const wchar_t       *GetBaseClassName2() const
    ...
    static wxClassInfo        *FindClass(const wchar_t *className);
...
private:
    ...
    static wxClassInfo *GetBaseByName(const wchar_t *name);

private:
    wxClassInfo(const wxClassInfo&);
    wxClassInfo& operator=(const wxClassInfo&);
...
protected:
 ...
 wxObject *wxCreateDynamicObject(const wchar_t *name);
...
#define wxIMPLEMENT_CLASS_COMMON(name, basename, baseclsinfo2, func)          \
    wxClassInfo name::ms_classInfo(_T(#name),                                \
...
private:                                        \
    static const std::wstring sm_className;         \
public:                                         \
    name##PluginSentinel();                     \
    ~name##PluginSentinel();                    \
};                                              \
...
#define _IMPLEMENT_DL_SENTINEL(name)                                \
 const std::wstring name::name##PluginSentinel::sm_className(#name);    \
...
/*
    ...
    _WX_WANT_NEW_SIZET_WXCHAR_INT             = void *operator new (size_t size, wchar_t *fileName = 0, int lineNum = 0)
    ...
    _WX_WANT_DELETE_VOID_WXCHAR_INT           = void operator delete(void *buf, wchar_t*, int)
    _WX_WANT_ARRAY_NEW_SIZET_WXCHAR_INT       = void *operator new[] (size_t size, wchar_t *fileName , int lineNum = 0)
    _WX_WANT_ARRAY_DELETE_VOID                = void operator delete[] (void *buf)
    _WX_WANT_ARRAY_DELETE_VOID_WXCHAR_INT     = void operator delete[] (void* buf, wchar_t*, int )
*/
...
public:
    ...
    void *operator new ( size_t size, const wchar_t *fileName = NULL, int lineNum = 0 );
#endif
    ...
    void operator delete ( void *buf, const wchar_t*, int );
#endif
...
public:
    ...
    void SetProperty (const wchar_t *propertyName, const wxxVariant &value);
    wxxVariant GetProperty (const wchar_t *propertyName) const ;
...
private :
    ...
    void RemoveProperty( const wchar_t *propertyName ) ;

    // renames an existing runtime-property
    void RenameProperty( const wchar_t *oldPropertyName , const wchar_t *newPropertyName ) ;
