...
//
// _WX_ERROR_REMOVE is needed to resolve the name conflict between the _T()
// macro and T typedef: we can't use _T() inside WX_DEFINE_ARRAY!

#define _WX_ERROR_REMOVE _T("removing inexisting element in wxArray::Remove")
...
public:                                                               \
  ...
  void Remove(T lItem)                                                \
    { int iIndex = Index(lItem);                                      \
      wxCHECK2_MSG( iIndex != std::wstring::npos, return,                    \
         _WX_ERROR_REMOVE);                                           \
